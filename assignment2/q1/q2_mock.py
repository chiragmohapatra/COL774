...
(test_x,test_y) = get_data_OnevsOne_sklearn(test_data)

(val_x,val_y) = get_data_OnevsOne_sklearn(val_data)

def OnevsOne_sklearn():
    ...
    print('Test Accuracy(OnevsOne model) for sklearn: ' , score*100 , '%')
    test_confusion_matrix = confusion_matrix(test_y,test_pred)
    print('Test confusion matrix for sklearn:')
    ...
    print('Validation Accuracy(OnevsOne model) for sklearn: ' , score*100 , '%')
    val_confusion_matrix = confusion_matrix(val_y,test_pred)
    print('Validation confusion matrix for sklearn:')
...
perform_kfold(5,[0.00001,0.001,1,5,10])
...
print('--------------------------------------------')
print('Training gaussian kernel svm for labels 3(1) and 4(-1)')
Gaussian_param = GaussianKernel_svm(train_x,train_y)
print('Test Accuracy for gaussian kernel svm: ', calc_accuracy_Gaussian(Gaussian_param,gamma,test_x,test_y)*100 , '%')
print('Validation Accuracy for gaussian kernel svm: ', calc_accuracy_Gaussian(Gaussian_param,gamma,val_x,val_y)*100 , '%')
"""
