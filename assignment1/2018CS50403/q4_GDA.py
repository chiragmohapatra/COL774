...
def read_data():
    ...
    y_list = []
    with open(os.path.join(data_dir , 'q4y.dat') , 'r') as file: 
    ...
    x_list = []
    with open(os.path.join(data_dir , 'q4x.dat') , 'r') as file: 
...
if(question_part == 'a'):
    ...
    with open(os.path.join(out_dir , 'output_4a.txt'), 'w') as f:
        print('These are the parameters learnt on training model assuming parameter tying:' , file=f)
        print('phi = ' , my_model_tied.phi , file=f)

        print('mu0 = ' , my_model_tied.mu0 , file=f)

        print('mu1 = ', my_model_tied.mu1 , file=f)

        print('sigma = ', my_model_tied.sigma , file=f)
    ...
    for i, c in enumerate(np.unique(y)):
        plt.scatter(x_array[:,0][y==c],x_array[:,1][y==c],c=colors[i], marker=markers[i] , label = labels[i])
    plt.legend()
    plt.xlabel('x1(Fresh Water)')
    plt.ylabel('x2(Marine Water)')
    plt.title('Growth ring parameters in Salmon')
    plt.savefig(os.path.join(out_dir , 'output_4b.png'))
    plt.close()
    ...
    plt.xlabel('x1(Fresh Water) - Normalised')
    plt.ylabel('x2(Marine Water) - Normalised')
    plt.title('Growth ring parameters in Salmon')
    plt.savefig(os.path.join(out_dir , 'output_4c.png'))
    plt.close()
    ...
    with open(os.path.join(out_dir , 'output_4d.txt'), 'w') as f:
        print('These are the parameters learnt upon training model with general gda' , file=f)

        print('phi = ', my_model_gen.phi , file=f)
        print('mu0 = ', my_model_gen.mu0 , file=f)
        print('mu1 = ', my_model_gen.mu1 , file=f)
        print('sigma0 = ', my_model_gen.sigma0 , file=f)
        print('sigma1 = ', my_model_gen.sigma1 , file=f)
    ...
    plt.ylabel('x2(Marine Water) - Normalised')
    plt.title('Growth ring parameters in Salmon')
    plt.savefig(os.path.join(out_dir , 'output_4e.png'))
    plt.close()
